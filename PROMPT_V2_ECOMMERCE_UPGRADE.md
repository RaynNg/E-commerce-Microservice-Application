...
export default function ChatbotWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Xin chào! Tôi là trợ lý mua sắm của ShopMicro 🛍️\nBạn cần tư vấn sách, laptop hay quần áo gì không?'
    }
  ]);
        ...
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 
          ...
          <div className="bg-blue-600 text-white px-4 py-3 flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-xs text-blue-200">Tư vấn sách • Laptop • Thời trang</div>
            </div>
          </div>
                      ...
                      {msg.products.slice(0, 3).map(p => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-blue-600">{formatPrice(p.price)}</div>
                          </div>
                          <span>→</span>
                        </a>
                      ))}
              ...
              <div className="flex justify-start">
                <div className="bg-white rounded-2xl rounded-bl-none px-4 py-3 shadow-sm">
                  <div className="flex gap-1">
                    {[0, 1, 2].map(i => (
                      <span key={i} className="w-2 h-2 bg-blue-400 rounded-full 
                                               animate-bounce" 
...
*Prompt này dựa trực tiếp trên code thực trong repo RaynNg/E-commerce-Microservice-Application.*
*Stack: Django DRF + PostgreSQL + Neo4j + RabbitMQ + React/Vite + Anthropic Claude.*
